...
// ─── Frame assets ─────────────────────────────────────────────────────────────
//
// Desktop: 24 JPEGs — every 3rd of the 70 original GIF frames.
//   f00.jpg = frame_00,  f01.jpg = frame_03,  f02.jpg = frame_06 … f23.jpg = frame_69
//   Strictly increasing; covers the full approach from forest entry → house arrival.
//
// Mobile:  8 frames — the same sequence subsampled at indices [0,3,6,9,12,15,18,21].
//   Still monotonically forward; lighter on memory.
//
const DESKTOP_FRAMES = Array.from(
  { length: 24 },
  (_, i) => `/assets/scroll-sequence/f${String(i).padStart(2, "0")}.jpg`
);
const MOBILE_FRAME_IDX = [0, 3, 6, 9, 12, 15, 18, 21] as const;
const MOBILE_FRAMES    = MOBILE_FRAME_IDX.map(i => DESKTOP_FRAMES[i]);
...
// ─── Scroll indicator ────────────────────────────────────────────────────────
function ScrollIndicator({ opacity, label }: { opacity: MotionValue<number>; label: string }) {
  return (
    <motion.div
      style={{ opacity }}
      ...
      aria-hidden="true"
    >
      <p className="font-sans text-offwhite/50 text-xs tracking-[0.2em] uppercase">{label}</p>
...
// ─── Progress dots ────────────────────────────────────────────────────────────
function ProgressDots({ total, scrollProgress }: { total: number; scrollProgress: MotionValue<number> }) {
  const [active, setActive] = useState(0);
  useMotionValueEvent(scrollProgress, "change", (v) => {
    setActive(Math.min(Math.floor(v * total), total - 1));
  });
  return (
    <div className="absolute right-5 top-1/2 -translate-y-1/2 flex flex-col gap-2 z-10" aria-hidden="true">
      {Array.from({ length: total }, (_, i) => (
        <div
          ...
          style={{
            width:  i === active ? 6 : 4,
            height: i === active ? 6 : 4,
            background: i === active ? "rgba(144,180,136,0.9)" : "rgba(245,243,238,0.3)",
          }}
        />
      ))}
    </div>
...
export function ScrollJourney() {
  ...
  // Ensures every frame is decoded into the browser's image cache before the
  // user scrolls to it — making the z-index swap feel instant.
  const frames = isMobile ? MOBILE_FRAMES : DESKTOP_FRAMES;
  useEffect(() => {
    if (reduced) return;
    const allFrames = [...MOBILE_FRAMES, ...DESKTOP_FRAMES];
  ...
  // ── Double-buffer — z-index swap (NO crossfade) ───────────────────────────
  //
  // WHY z-index instead of opacity:
  //   With opacity transitions, rapid scroll fires multiple swap calls within
  //   the transition window. Each call reverses mid-transition, causing the
  //   "bounce" (A fading out suddenly snaps back to 1 while B reverses to 0).
  //   Z-index swap is instant: the new frame appears on top without competing
  //   opacity animations. Since all frames are preloaded, the swap is visually
  //   seamless.
  //
  const imgARef    = useRef<HTMLImageElement>(null);
  const imgBRef    = useRef<HTMLImageElement>(null);
  const bufRef     = useRef<{ active: "A" | "B"; lastFrame: number }>({ active: "A", lastFrame: -1 });
  ...
  const swapFrame = useCallback(() => {
    ...
    // Monotonic forward mapping: scroll 0→1 maps to frame 0→(total-1).
    // Math.floor(p * total) overshoots at p=1.0, so we clamp to total-1.
    ...
    if (bufRef.current.active === "A") {
      // Load new frame into B, then bring B on top instantly (no opacity anim)
      imgB.src                = localFrames[idx];
      imgB.style.zIndex       = "2";
      imgA.style.zIndex       = "1";
      bufRef.current.active   = "B";
    } else {
      // Load new frame into A, then bring A on top
  ...
  // ── Cinematic zoom — the approach effect ─────────────────────────────────
  //
  // Scale goes 1.0 → 1.14 over the full scroll journey, simulating a slow
  // camera push-in toward the house. Combined with the frame sequence (which
  // already shows a physical approach), this makes the arrival feel immersive.
  //
  // transformOrigin "50% 55%": zoom anchors slightly below center so the path
  // and house stay in frame as the scale grows (the horizon is typically above
  // the vertical midpoint in architecture shots).
  //
  const imageScale = useTransform(
    scrollYProgress,
    [0, 1],
    reduced ? [1, 1] : [1.0, 1.14]
  );

  // ── Supporting motion values ─────────────────────────────────────────────
  ...
  return (
    <section
      ...
      className="relative"
      aria-label={lang === "es" ? "Viaje arquitectónico AMBIENTAR" : "AMBIENTAR architectural journey"}
    >
      {/* ── Sticky viewport ── */}
      <div className="sticky top-0 h-[100dvh] overflow-hidden">

        {/* ── Zooming image layer ──────────────────────────────────────────────
            The motion.div carries the CSS scale transform. Using will-change:transform
            promotes this layer to the GPU and ensures the scale animates without
            triggering layout recalculation on the rest of the page.
        ── */}
        ...
        >
          {/* Poster — always-visible SSR fallback */}
          <img
            src={POSTER}
            alt=""
            aria-hidden="true"
          ...
          {/* Buffer A — starts on top (z=2), src = first frame */}
          <img
            ref={imgARef}
            src={frames[0]}
            alt=""
          ...
          {/* Buffer B — starts beneath (z=1), src = second frame preloaded */}
          <img
            ref={imgBRef}
        ...
        {/* Sits above the zoom wrapper in DOM order → naturally on top */}
        <motion.div
          aria-hidden="true"
        ...
        {/* ── Scroll indicator ── */}
        <ScrollIndicator opacity={scrollHintOpacity} label={t.scrollHint} />
